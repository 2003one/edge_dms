...
| | Phase 1 — Simulation | Phase 2 — Physical vehicle |
|---|---|---|
...
> Phase 1 (TurtleBot3 in Gazebo, SLAM Toolbox + Nav2) remains fully runnable — see [Run the simulation](#3--run-the-simulation-phase-1). If you have a screenshot of the Gazebo run, drop it in `docs/images/turtlebot_sim.png` and embed it here.
...
│   └── src/dms_controller/     # bridge package (dms_bridge_node)
├── esp8266_firmware/           # ── Physical vehicle ── Arduino C++ firmware
│   └── car_firmware.ino        # Wi-Fi web server, active-LOW motor control, IMU + bias calibration
...
> Build artifacts (`dms_ws/build|install|log`) and the Pi virtualenv are not committed — regenerate them locally (see [Getting started](#getting-started)).
...
**Linux host** — Ubuntu 24.04, ROS2 **Jazzy**, Gazebo, TurtleBot3 packages:
...
### 2 · Linux host — ROS2 workspace (`dms_ws/`)

```bash
source /opt/ros/jazzy/setup.bash
cd ~/dms_ws
colcon build --packages-select dms_controller
source install/setup.bash        # add to ~/.bashrc for convenience
```

If creating the package from scratch: `ros2 pkg create --build-type ament_python dms_controller --dependencies rclpy geometry_msgs std_msgs`, place the node at `dms_controller/dms_bridge_node.py` (exposing `main()`), and register `dms_bridge_node = dms_controller.dms_bridge_node:main` under `console_scripts` in `setup.py`.

### 3 · Run the simulation (Phase 1)
...
# Terminal 2 — teleop, REMAPPED to /cmd_vel_teleop
export TURTLEBOT3_MODEL=burger
ros2 run turtlebot3_teleop teleop_keyboard \
    --ros-args --remap /cmd_vel:=/cmd_vel_teleop

# Terminal 3 — DMS bridge
ros2 run dms_controller dms_bridge_node
```

> **Why teleop is remapped:** the bridge subscribes to `/cmd_vel_teleop`, applies the current driver-state policy (pass-through / ×0.5 / zero), and republishes on `/cmd_vel` — the DMS *gates* manual control instead of fighting it. In DANGER (10 s sustained) it publishes zero-velocity at 10 Hz, overriding input entirely.

### 4 · Run the physical car (Phase 2)

1. **Flash** `esp8266_firmware/car_firmware.ino` with your Wi-Fi credentials (Arduino IDE → NodeMCU 1.0).
